...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train a FastText model or extract rules.')
    ...
    parser.add_argument('--extract_rules', action='store_true', help='If given, rules will be extracted.')
    ...
    if args.extract_rules:
        if not args.spec_path or not args.settings or not args.model_name:
            parser.error("--extract_rules requires --spec_path, --settings and --model_name.")
        # Parse the OpenAPI specification
        operation_data = parse_parameters(args.spec_path)
        # Create an instance of RuleExtractor
        rule_extractor = RuleExtractor(args.settings, args.model_name)
        # Load previous results, if they exist

        prev_results = {}
        results = {}
        # Pass the operation data to the rule extractor
        for operation, parameters in operation_data.items():
            param_names = [param["name"] for param in parameters]
            for param in parameters:
                # Extract rules for the parameter
                ...
                if param_key in prev_results:
                    ...
                    if operation not in results:
                        results[operation] = {}
                    prev_results[param_key] = rule_extractor.extract_rules(*param_key)
                    results[operation][param["name"]] = prev_results[param_key]
                if param["properties"] is not None:
                    # Extract rules for each property
                    for prop_name, prop_info in param["properties"].items():
                        ...
                        if prop_key in prev_results:
                            ...
                            if operation not in results:
                                results[operation] = {}
                            prev_results[prop_key] = rule_extractor.extract_rules(*prop_key)
                            results[operation][param["name"]] = prev_results[param_key]
        # Save the results
